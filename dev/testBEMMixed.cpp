...
auto dPhiDn = [](double nr, double nz, double r, double z) {
    ...
    dPhiDn_ = nr * (-r) + nz * (2. * z);
    return dPhiDn_; // nr * (-r) + nz * (2. * z);
};                  //eqn 7.88: dphi/dn

int main()
{

    for (int global = 5; global < 19 + 1; global++)
    {
        ...
        knot[0] = numericTools::Geometry2D::benchmarkShape(0.0, M_PI / 2, N);
        //knot[1] = numericTools::Geometry2D::circle(knot[0](knot[0].rows() - 1, 0), knot[0](knot[0].rows() - 1, 1), N,
        //                                         numericTools::CircleType::DownWrap);
        knot[1] = numericTools::Geometry2D::line(knot[0](knot[0].rows() - 1, 0), knot[0](knot[0].rows() - 1, 1), 0.0, 0.0, N);

        bem2D::BoundaryElement bem[2];

        bem[0].sp().init(knot[0]);
        bem[0].sp().setNode();
        bem[0].sp().setBC(0, spline::BCType::Odd, spline::BCType::Even, 0, 0, 0, 0);
        bem[0].sp().setBC(1, spline::BCType::Even, spline::BCType::Odd, 0, 0, 0, 0);
        // bem[0].sp().setBC(0, spline::BCType::Odd, spline::BCType::Mix, 0, 0,
        ...
        bem[1].sp().setNode();
        bem[1].sp().setBC(0, spline::BCType::Odd, spline::BCType::Odd, 0, 0, 0, 0);
        bem[1].sp().setBC(1, spline::BCType::Even, spline::BCType::Even, 0, 0, 0, 0);
        ...
        bem[0].sp().write("../resources/testBEMMixed/spline0.txt");
        bem[1].sp().write("../resources/testBEMMixed/spline1.txt");
        ...
        Eigen::VectorXd errorDirichlet(n[0]), errorNeumann(n[1]);
