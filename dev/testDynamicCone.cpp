...
int main()
{
    ...
    double b0 = 1;
    double rCut = 100.0;
    dynamicCone::PatchedCone patchedCone(c1, b0, rCut, 100, 400, 200);

    int nTotal = patchedCone.parameter().n[0] + patchedCone.parameter().n[2];
    ...
    bem2D::BoundaryElement::assembleMatrix(patchedCone._bem[0], patchedCone._bem[2], S, D);
    bem2D::BoundaryElement::assembleMatrix(patchedCone._bem[2], patchedCone._bem[0], S, D);
    bem2D::BoundaryElement::assembleMatrix(patchedCone._bem[2], patchedCone._bem[2], S, D);
    ...
    Eigen::VectorXd given = patchedCone.setVacuumBC(patchedCone._bem[0], patchedCone._bem[2]);

    patchedCone.solveBVP(patchedCone._bem[0], given, dynamicCone::BoundaryType::Vacuum, S, D, p, q);

    Eigen::MatrixXd output;
    output.resize(patchedCone.parameter().n[0], 3);

    for (int k = 0; k < patchedCone.parameter().n[0]; k++)
    {
        output(k, 0) = patchedCone._bem[0].node()[k].x;
        output(k, 1) = q(k);
        output(k, 2) = p(k);
    }
    bem2D::io::IOEigen::write("../resources/testDynamicCone/output.txt", output);
